...
class HoloGraph(nn.Module):
    ...
    def __init__(
        ...
        for l in range(self.L):
            ...
            kuramoto_layer = Kuramoto_Solver(
                n=n,
                ch=ch,
                J=self.J[l],
                c_norm=c_norm,
            ...
            self.layers.append(nn.ModuleList([kuramoto_layer, readout]))
        ch = ch_next
    ...
    def feature(self, inp, inp_fc, inp_sc):
        ...
        if inp_sc.dim() == 2 and inp_sc.size(0) == 2:
            sc_dense = to_dense_adj(inp_sc, max_num_nodes=N).float()
        elif inp_sc.dim() == 2:
            sc_dense = inp_sc.unsqueeze(0).float()
        else:
            sc_dense = inp_sc.float()
    ...
    def forward(self, input, input_fc, input_sc, return_xs=False, return_es=False, return_features=False):
        ...
        # # This is for graph-level prediction, not node-level
        # logits = self.out_pred_graph(final_y)  
...
class HoloBrain(nn.Module):
    """
    HoloBrain: Physics-Informed Graph Neural Network (Specialized for Brain Networks).
    ...
    def __init__(
        ...
        if feature_dim is None:
            raise ValueError("feature_dim is required.")
        ...
        # ============================================================
        # 1. Attending Memory Encoder (y) Definition
        # ============================================================
        
        # [Homo Branch]: GCN (Input: feature_dim)
        self.encoder_y_homo_1 = GCNConv(feature_dim, ch) 
        self.encoder_y_homo_2 = GCNConv(ch, ch)
        self.norm_y_1 = nn.LayerNorm(ch)
        self.norm_y_2 = nn.LayerNorm(ch)
        
        # [Non-Homo/Brain Branch]: MultiConv1D (Input: feature_dim)
        self.encoder_y_non_homo = MultiConv1D(in_channels=self.feature_dim, out_channels=self.feature_dim, num_convs=self.gst_total)
        
        # [Legacy Projections]
        self.proj_y_legacy_1 = nn.Sequential(
            nn.Linear(self.num_nodes, ch), # Legacy params kept for seed matching if needed
        )
        self.proj_y_legacy_2 = nn.Linear(self.feature_dim * self.gst_total, ch)
        
        # [Main Projection for y (Brain)]
        self.proj_y = nn.Conv1d(self.feature_dim * self.gst_total, ch, kernel_size=1, stride=1, padding=0)

        # ============================================================
        # 2. Oscillator Initialization (x0) Definition
        # ============================================================
        
        # [Main Projection for x0 (GST)]
        self.proj_x0 = nn.Conv1d(self.feature_dim * self.gst_total, ch, kernel_size=1, stride=1, padding=0)
    ...
    def feature(self, inp, inp_fc, inp_sc):
        ...
        # 1. Attending Memory 'y'
        if self.homo:
            # GCN Branch: [N, F]
            x_in = inp.squeeze(0).transpose(0, 1)
            y = self.encoder_y_homo_1(x_in, inp_sc)
            y = self.norm_y_1(y)
            y = F.relu(y)
            y = self.dropout(y)
            
            identity = y
            y = self.encoder_y_homo_2(y, inp_sc)
            y = self.norm_y_2(y)
            if self.use_residual:
                y = y + identity
            y = F.relu(y)
            ...
            # Brain Branch: [B, F, N]
            # MultiConv1D convolving over Features
            y = self.encoder_y_non_homo(inp) 
            y = y.flatten(start_dim=2)
            
            # Project to y [B, C, N]
            y = self.proj_y(y.transpose(1,2))

        # 2. Oscillator Init 'x' (GST)
        # Robust N inference
        if inp.dim() == 2:
            N = inp.size(0)
            x_gst_in = inp.t().unsqueeze(0) 
        elif inp.dim() == 3:
            # Brain: [B, F, N]. GST usually expects [B, F, N]
            if inp.size(1) == self.feature_dim: 
                x_gst_in = inp 
                N = inp.size(2)
            else:
                x_gst_in = inp.transpose(1, 2)
                N = inp.size(1)
        else:
            N = inp_sc.shape[-1]
            x_gst_in = inp.contiguous()

        # Ensure dense adjacency
        if inp_sc.dim() == 2 and inp_sc.size(0) == 2:
            sc_dense = to_dense_adj(inp_sc, max_num_nodes=N).float()
        elif inp_sc.dim() == 2:
            sc_dense = inp_sc.unsqueeze(0).float()
            ...
            sc_dense = inp_sc.float()
        graph_sc = sc_dense.squeeze(0) if (isinstance(self.J, list) and any(j == "adj" for j in self.J)) or (isinstance(self.J, str) and self.J == "adj") else inp_sc

        # GST Forward
        x = self.gst(x_gst_in, sc_dense)
        x = x.flatten(start_dim=2)
        
        # Project GST output to x0
        if self.homo:
             x = self.proj_x0(x.transpose(1,2)).transpose(1, 2)
        else:
        ...
        for i, (kblock, ro) in enumerate(self.layers):
            # Pass Q instead of T, y instead of c
            _xs, _es, save_x = kblock(x, y, graph_sc, Q=self.Q[i], gamma=self.gamma)
            x = _xs[-1]
            x = ro(x, graph_sc)
            xs.append(_xs)
            es.append(_es)
    ...
    def forward(self, input, input_fc, input_sc, return_xs=False, return_es=False, return_features=False):
        ...
        final_y, x, xs, es, x_L, saved_y, pre_ode, post_ode = self.feature(input, input_fc, input_sc)
        
        # For Brain Classification, use graph pooling output
        logits = self.out_pred_graph(final_y) 
        ...
        if len(ret) == 1:
            if return_features:
                return ret[0], x, saved_y, pre_ode, post_ode
            else:
                return ret[0], x, saved_y
        
        if return_features:
            return ret, x, saved_y, pre_ode, post_ode 
        else:
            return ret, x, saved_y
